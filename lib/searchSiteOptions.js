...
async function getRelatedVideos(page, relatedVideoSelector, relatedVideoTitle) {
    return page.evaluate((selector) => {
        ...
        .then(async resultsLength => {
            let relatedVidsResults = [];

            let len = resultsLength <= 14 ? resultsLength : 14;
            for (let i = 0; i < len; i++) {
                await page.hover(`${relatedVideoSelector}:nth-of-type(${i + 1})`)
                    .catch(e => console.error("Couldn't find the related video selector of index:" + (i + 1)));

                let res = await page.evaluate((selector, relatedVideoTitle, idx) => {
                    let r = document.querySelector(`${selector}:nth-of-type(${idx + 1})`);

                    let link = r.querySelector("a")?.href;
                    let image = r.querySelector("img")?.src;
                    let video = r.querySelector("video")?.src ? r.querySelector("video")?.src : r.querySelector("video source")?.src
                    let title = relatedVideoTitle ? r.querySelector(relatedVideoTitle).textContent.trim() : null;

                    return { link, image, video, title };
                }, relatedVideoSelector, relatedVideoTitle, i)
